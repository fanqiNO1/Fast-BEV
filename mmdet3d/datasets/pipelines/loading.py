...
class LoadMultiViewImageFromFiles_BEVDet(object):
    ...
    def pil_img_pad(self, pil_img, top, right, bottom, left, color):
    ...
    def img_transform_core(self, img, resize_dims, crop, flip, rotate, pad):
        ...
        top, right, bottom, left = pad[0]
        pad_color = pad[1]
        img = self.pil_img_pad(img, top, right, bottom, left, pad_color)
        return img
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam in cams:
            ...
            # augmentation (resize, crop, horizontal flip, rotate, pad)
            resize, resize_dims, crop, flip, rotate, pad = self.sample_augmentation(H=img.height,
                                                                                    W=img.width,
                                                                                    flip=flip,
                                                                                    scale=scale)
    ...
    def __call__(self, results):
        ...
        # with open('./jupyter/results.pkl', 'wb') as f: pickle.dump(results, f, protocol=pickle.HIGHEST_PROTOCOL)
        return results
