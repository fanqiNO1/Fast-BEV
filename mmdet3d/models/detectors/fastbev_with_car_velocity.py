...
class FastBEVWithCarVelocity(BaseDetector):
    def __init__(
        self,
        backbone,
        neck,
        ...
        self.loss_car_velocity_weight = loss_car_velocity_weight
        ...
        self.car_velocity_head = nn.Sequential(
            nn.Conv2d(192, 2, 1),
    ...
    def forward_train(
        self, img, img_metas, gt_bboxes_3d, gt_labels_3d, gt_car_velocity, gt_bev_seg=None, **kwargs
    ):
        ...
        # loss_car_velocity
        x_car_velocity = self.car_velocity_head(feature_bev[0]).squeeze(-1).squeeze(-1)
        gt_car_velocity = gt_car_velocity.to(x_car_velocity.dtype)
        loss_car_velocity = {
            'loss_car_velocity': self.loss_car_velocity_weight * F.mse_loss(x_car_velocity, gt_car_velocity)
        }
        losses.update(loss_car_velocity)
    ...
    def simple_test(self, img, img_metas):
        ...
        # car velocity
        x_car_velocity = self.car_velocity_head(feature_bev[0]).squeeze(-1).squeeze(-1)
        bbox_results[0]['car_velocity'] = x_car_velocity
